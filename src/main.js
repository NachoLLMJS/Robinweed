...
async function connectPlayerWallet(){
  if(state.mode==='spectator'){showToast('OFFCHAIN DEMO · WALLET DISABLED');return null;}
...
async function buySelectedSeeds(){
  if(state.mode==='spectator'){state.seeds=buySpectatorSeedPack(state.seeds);refresh();showToast(`${vendorSelectedProduct.ticker} DEMO SEEDS +${vendorSelectedProduct.packSize} · OFFCHAIN ONLY`);return;}
...
function renderPropertyPanel(){
  ...
  if(state.mode==='spectator'){ui.propertyMessage.textContent=access==='OWNER'?'This demo property is stored only in this session. Enter and use its grow stations.':'Claim this property in the local demo. No wallet, token or transaction is used.';ui.propertyAction.textContent=access==='OWNER'?'OPEN DEMO PROPERTY':'CLAIM IN OFFCHAIN DEMO';ui.propertyAction.disabled=access==='OCCUPIED';return;}
...
const particles=[];
function openPanel(panel){if(state.mode==='spectator'&&panel===ui.wallet){showToast('OFFCHAIN DEMO · WALLET DISABLED');return;}document.exitPointerLock();panel.classList.remove('hidden');if(panel===ui.vendor)setVendorAnimation(vendorClipForPanel(true));}
