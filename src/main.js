...
import { createSerializedAuthenticator, MultiplayerClient } from './multiplayerClient.js';
...
async function syncMultiplayerLocation(){
  ...
  try{await ensureAuthenticatedSession(wallet);if(generation!==multiplayerGeneration||state.location!=='street'||state.walletAddress?.toLowerCase()!==wallet.toLowerCase())return;const client=new MultiplayerClient({onSnapshot:onMultiplayerSnapshot,onClose:(_event,{willReconnect}={})=>{if(multiplayerClient!==client)return;if(!willReconnect)multiplayerClient=null;for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();}});client.connect();multiplayerClient=client;showToast('OUTSIDE MULTIPLAYER CONNECTED');}catch(error){multiplayerClient=null;console.warn('Stockdealer · outside multiplayer unavailable',error);}finally{multiplayerConnecting=false;}
}
