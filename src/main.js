...
async function syncMultiplayerLocation(){
  if(state.location!=='street'||!state.walletAddress){multiplayerGeneration++;multiplayerClient?.close();multiplayerClient=null;localServerReady=false;if(state.walletAddress)setMultiplayerStatus('PRIVATE');else setMultiplayerStatus('OFF');for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();return;}
  ...
  try{await ensureAuthenticatedSession(wallet);if(generation!==multiplayerGeneration||state.location!=='street'||state.walletAddress?.toLowerCase()!==wallet.toLowerCase())return;localServerReady=false;const client=new MultiplayerClient({onSnapshot:onMultiplayerSnapshot,onStatus:status=>{if(multiplayerClient!==client)return;if(status.state==='connecting')setMultiplayerStatus('CONNECTING');else if(status.state==='socket-open')setMultiplayerStatus('JOINING');else if(status.state==='online')setMultiplayerStatus(String(status.players));else if(status.state==='closed')setMultiplayerStatus(status.willReconnect?'RECONNECTING':`CLOSED ${status.code}`);},onClose:(_event,{willReconnect}={})=>{if(multiplayerClient!==client)return;localServerReady=false;if(!willReconnect)multiplayerClient=null;for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();}});multiplayerClient=client;client.connect();}catch(error){multiplayerClient=null;setMultiplayerStatus('AUTH ERROR');console.warn('Stockdealer · outside multiplayer unavailable',error);}finally{multiplayerConnecting=false;}
}
