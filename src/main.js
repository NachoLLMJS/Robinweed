...
import { assignSessionPlayerSkin } from './playerSkinState.js';
...
import { authenticateRealtime, MultiplayerClient } from './multiplayerClient.js';
...
let multiplayerClient=null,multiplayerConnecting=false,lastMultiplayerInputAt=0,multiplayerGeneration=0,authenticatedWallet=null;
function makeRemotePlayer(address){
  const group=new THREE.Group();
  const bodyMaterial=new THREE.MeshStandardMaterial({color:0x17201a,roughness:.88,flatShading:true});
  const skinMaterial=new THREE.MeshStandardMaterial({color:0xb87957,roughness:.9,flatShading:true});
  const body=new THREE.Mesh(new THREE.CapsuleGeometry(.3,.84,2,6),bodyMaterial);body.position.y=1.02;body.castShadow=true;group.add(body);
  const head=new THREE.Mesh(new THREE.IcosahedronGeometry(.24,1),skinMaterial);head.position.y=1.72;head.castShadow=true;group.add(head);
  ...
  group.userData.target=new THREE.Vector3();scene.add(group);return group;
}
function onMultiplayerSnapshot(snapshot){
  const seen=new Set();
  for(const player of snapshot.players){if(!player?.userId||![player.x,player.z,player.yaw].every(Number.isFinite))continue;if(player.address?.toLowerCase()===state.walletAddress?.toLowerCase()){if(state.location==='street'){camera.position.x=player.x;camera.position.z=player.z;}continue;}seen.add(player.userId);let group=remotePlayers.get(player.userId);if(!group){group=makeRemotePlayer(player.address);remotePlayers.set(player.userId,group);}group.userData.target.set(player.x,.02,player.z);group.userData.targetYaw=player.yaw;group.visible=state.location==='street';}
  for(const [id,group] of remotePlayers)if(!seen.has(id)){scene.remove(group);remotePlayers.delete(id);}
}
async function ensureAuthenticatedSession(wallet){if(authenticatedWallet?.toLowerCase()===wallet?.toLowerCase())return;await authenticateRealtime({ethereum:globalThis.ethereum,account:wallet});if(state.walletAddress?.toLowerCase()!==wallet.toLowerCase())throw new Error('ACCOUNT_CHANGED_DURING_AUTH');authenticatedWallet=wallet;}
async function syncMultiplayerLocation(){
  if(state.location!=='street'||!state.walletAddress){multiplayerGeneration++;multiplayerClient?.close();multiplayerClient=null;for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();return;}
  ...
  try{await ensureAuthenticatedSession(wallet);if(generation!==multiplayerGeneration||state.location!=='street'||state.walletAddress?.toLowerCase()!==wallet.toLowerCase())return;const client=new MultiplayerClient({onSnapshot:onMultiplayerSnapshot,onClose:(_event,{willReconnect}={})=>{if(multiplayerClient!==client)return;if(!willReconnect)multiplayerClient=null;for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();}});multiplayerClient=client;client.connect();showToast('OUTSIDE MULTIPLAYER CONNECTED');}catch(error){console.warn('Stockdealer · outside multiplayer unavailable',error);}finally{multiplayerConnecting=false;}
}
...
async function connectPlayerWallet(){
  ...
  try{const accounts=await globalThis.ethereum.request({method:'eth_requestAccounts'});const wallet=accounts?.[0];if(!/^0x[0-9a-f]{40}$/i.test(wallet??''))throw new Error('Invalid wallet');state.walletAddress=wallet;ui.walletAddress.textContent=wallet;ui.walletButton.textContent=`WALLET — ${compactWallet(wallet)}`;showToast('WALLET CONNECTED · NO TRANSACTION SENT');await ensureAuthenticatedSession(wallet);await reconcileEconomyJournal({ethereum:globalThis.ethereum,account:wallet}).catch(error=>{console.warn('Stockdealer · economy reconciliation required',error);showToast('ECONOMY RECONCILIATION REQUIRED');});await refreshOnchainState().catch(error=>console.warn('Stockdealer · onchain state unavailable',error));await syncMultiplayerLocation();return wallet;}catch(error){showToast(error?.code===4001?'WALLET CONNECTION REJECTED':'WALLET CONNECTION FAILED');return null;}
}
function clearWalletContext(){multiplayerGeneration++;multiplayerClient?.close();multiplayerClient=null;authenticatedWallet=null;state.walletAddress=null;state.onchainSnapshot=null;state.properties=new Map(HOUSE_PROPERTIES.map(property=>[property.id,property]));state.activePropertyId=null;state.selectedPropertyId=null;if(state.location==='house-interior'){state.location='street';camera.position.set(0,1.68,9.55);yaw=Math.PI;}for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();ui.walletAddress.textContent='NOT CONNECTED';ui.walletButton.textContent='WALLET — NOT CONNECTED';refresh();}
async function handleAccountsChanged(accounts){clearWalletContext();const wallet=accounts?.[0];if(!/^0x[0-9a-f]{40}$/i.test(wallet??''))return;state.walletAddress=wallet;ui.walletAddress.textContent=wallet;ui.walletButton.textContent=`WALLET — ${compactWallet(wallet)}`;try{await ensureAuthenticatedSession(wallet);await reconcileEconomyJournal({ethereum:globalThis.ethereum,account:wallet}).catch(error=>{console.warn('Stockdealer · economy reconciliation required',error);showToast('ECONOMY RECONCILIATION REQUIRED');});await refreshOnchainState();await syncMultiplayerLocation();}catch(error){console.warn('Stockdealer · wallet context refresh failed',error);}}
...
function closePanels(){ui.vendor.classList.add('hidden');ui.property.classList.add('hidden');ui.wallet.classList.add('hidden');ui.portal.classList.add('hidden');househeadPanel='closed';renderHouseheadPanel();state.pendingPortal=null;setVendorAnimation(vendorClipForPanel(false));}
...
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.04,(now-last)/1000);last=now;let playerMoving=false;
  ...
  if(state.location==='street'&&multiplayerClient&&now-lastMultiplayerInputAt>=100){lastMultiplayerInputAt=now;const buttons=[];if(input.isHeld('KeyW'))buttons.push('forward');if(input.isHeld('KeyS'))buttons.push('backward');if(input.isHeld('KeyA'))buttons.push('left');if(input.isHeld('KeyD'))buttons.push('right');if(input.isHeld('ShiftLeft'))buttons.push('run');multiplayerClient.sendInput(buttons,yaw);}
  for(const group of remotePlayers.values()){group.visible=state.location==='street';group.position.lerp(group.userData.target,.28);const targetYaw=group.userData.targetYaw??group.rotation.y;group.rotation.y+=Math.atan2(Math.sin(targetYaw-group.rotation.y),Math.cos(targetYaw-group.rotation.y))*.28;}
