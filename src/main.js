...
import { trailerButtonsVisible } from './loaderState.js';
...
import { canEquipOnchainSeedSlot, visibleOnchainSeedSlots } from './onchainSeedInventory.js';
...
import { beginDisplayWatering, beginVisualWatering, advanceVisualWatering, visualWaterDrops } from './visualWatering.js';
import { SPECTATOR_IDENTITY, buySpectatorSeedPack, createSpectatorSession, purchaseSpectatorProperty, spectatorPropertyAccess, waterSpectatorPlant } from './spectatorMode.js';
import { BLUNT_BUD_COST, pickBluntTicker, rollBlunt } from './blunt.js';
import { beginHigh, HIGH_TOTAL_MS, sampleHigh, smokeWindFor } from './bluntHighState.js';
...
function updateVendorEconomyButton(){if(document.body.classList.contains('spectator-mode')){vendorBuyButton.disabled=false;vendorBuyButton.textContent=`ADD ${vendorSelectedProduct.packSize} DEMO SEEDS`;return;}const hood=vendorSelectedProduct.ticker==='HOOD';const ready=economyConfig.economyActive&&!hood;vendorBuyButton.disabled=!ready;vendorBuyButton.textContent=hood?'HOOD STOCK TOKEN UNAVAILABLE':ready?`BUY ${vendorSelectedProduct.packSize} SEEDS ONCHAIN`:'ECONOMY NOT ACTIVE';}
...
function selectVendorProduct(product){vendorSelectedProduct=product;document.querySelector('#vendorSelectedTicker').textContent=product.ticker;const demo=document.body.classList.contains('spectator-mode');document.querySelector('#vendorSwapRoute').textContent=demo?`${product.ticker} DEMO INVENTORY · NO PAYMENT`:`60% $STOCKDEALER → ${product.ticker} REWARD RESERVE`;document.querySelector('.vendor-routing small').textContent=demo?'Local session only. No wallet request, token spend, reward or transaction.':'Purchase disabled until token, router and funded reward vault are verified.';document.querySelectorAll('.vendor-seed-option').forEach(button=>button.classList.toggle('active',button.dataset.ticker===product.ticker));updateVendorEconomyButton();loadVendorPackPreview(product);}
...
// Two presentation-only grow displays. They stay outside `pots` and V4 cultivation:
// no plot IDs, transactions, persistence, rewards, inventory or ownership state.
...
const displayShelfLed=new THREE.MeshStandardMaterial({color:0xd9ffcf,emissive:0xaaff91,emissiveIntensity:2.8,roughness:.35,toneMapped:false});
const displayShelfPool=new THREE.MeshBasicMaterial({color:0x9dff7a,transparent:true,opacity:.2,depthWrite:false,blending:THREE.AdditiveBlending,toneMapped:false});
...
for(const bank of DISPLAY_SHELF_BANKS){const light=new THREE.PointLight(0xb8ff9c,1.15,3.1,1.8);light.position.set(bank.center[0],1.42,bank.center[2]+.08);light.userData={decorativeDisplay:true,displayBank:bank.id,displayKind:'localLight'};scene.add(light);legacyWarehouseShell.push(light);displayShelfRoots.push(light);}
const displayPotGeometry=new THREE.CylinderGeometry(.34,.27,.46,10);
const displaySoilGeometry=new THREE.CylinderGeometry(.29,.29,.025,12);
const displayStemGeometry=new THREE.CylinderGeometry(.025,.04,.72,7);
...
const displayLeafMaterial=new THREE.MeshStandardMaterial({color:0x55ad4b,emissive:0x102b0d,emissiveIntensity:.34,roughness:.76,side:THREE.DoubleSide});
const displayPlantHits=[];
const displayPlantMounts=displayPlantPlacements().map(config=>{
  const mount=new THREE.Group();mount.position.set(...config.position);mount.userData={decorativeDisplay:true,displayId:config.displayId,displayBank:config.bank,displayStage:config.stage};
  const visual=new THREE.Group();visual.scale.setScalar(config.scale);mount.add(visual);
  const pot=new THREE.Mesh(displayPotGeometry,displayPotMaterial);pot.position.y=.23;pot.castShadow=true;visual.add(pot);
  const soil=new THREE.Mesh(displaySoilGeometry,displaySoilMaterial);soil.position.y=.47;visual.add(soil);
  const stem=new THREE.Mesh(displayStemGeometry,displayLeafMaterial);stem.position.y=.78;visual.add(stem);
  for(const angle of [0,Math.PI/2,Math.PI,Math.PI*1.5]){const leaf=new THREE.Mesh(new THREE.ConeGeometry(.16,.58,5),displayLeafMaterial);leaf.position.set(Math.cos(angle)*.2,.82,Math.sin(angle)*.2);leaf.rotation.z=Math.PI/2;leaf.rotation.y=angle;visual.add(leaf);}
  const hit=new THREE.Mesh(new THREE.CylinderGeometry(.19,.19,.62,10),new THREE.MeshBasicMaterial({transparent:true,opacity:0,depthWrite:false}));hit.position.y=.31;hit.userData.interactive={type:'displayPlant',display:config};mount.add(hit);displayPlantHits.push(hit);
  scene.add(mount);legacyWarehouseShell.push(mount);displayShelfRoots.push(mount);return{mount,visual,hit,config};
});
function refreshDisplayPlantStage(stage){const template=plantTemplates.get(stage);if(!template)return;for(const entry of displayPlantMounts.filter(item=>item.config.stage===stage)){entry.visual.clear();const plant=template.clone(true);plant.userData.decorativeDisplay=true;plant.userData.displayId=entry.config.displayId;entry.visual.add(plant);}}
...
const bluntSpriteCanvas=document.createElement('canvas');bluntSpriteCanvas.width=64;bluntSpriteCanvas.height=64;const bluntSpriteContext=bluntSpriteCanvas.getContext('2d');const bluntSpriteGradient=bluntSpriteContext.createRadialGradient(32,32,0,32,32,32);bluntSpriteGradient.addColorStop(0,'rgba(255,255,255,1)');bluntSpriteGradient.addColorStop(.42,'rgba(255,255,255,.5)');bluntSpriteGradient.addColorStop(1,'rgba(255,255,255,0)');bluntSpriteContext.fillStyle=bluntSpriteGradient;bluntSpriteContext.fillRect(0,0,64,64);const bluntSpriteTexture=new THREE.CanvasTexture(bluntSpriteCanvas);
const bluntRoot=new THREE.Group();bluntRoot.visible=false;camera.add(bluntRoot);
const bluntPaperMaterial=new THREE.MeshStandardMaterial({color:0xcaa66a,roughness:.94,metalness:0});
const bluntWrapMaterial=new THREE.MeshStandardMaterial({color:0x754722,roughness:.9,metalness:0,emissive:0x4e321d,emissiveIntensity:.12});
const bluntAshMaterial=new THREE.MeshStandardMaterial({color:0x4a453d,roughness:1,metalness:0});
const bluntCherryMaterial=new THREE.MeshStandardMaterial({color:0xff5a12,roughness:.62,metalness:0,emissive:0xff4d05,emissiveIntensity:.35});
for(const [geometry,material,x] of [[new THREE.CylinderGeometry(.012,.016,.13,10),bluntWrapMaterial,-.03],[new THREE.CylinderGeometry(.013,.014,.035,10),bluntPaperMaterial,-.112],[new THREE.CylinderGeometry(.011,.012,.018,10),bluntAshMaterial,.044],[new THREE.CylinderGeometry(.009,.011,.009,10),bluntCherryMaterial,.057]]){const part=new THREE.Mesh(geometry,material);part.rotation.z=Math.PI/2;part.position.x=x;bluntRoot.add(part);}
const bluntEmberGlow=new THREE.Sprite(new THREE.SpriteMaterial({map:bluntSpriteTexture,color:0xff7a22,transparent:true,opacity:0,depthWrite:false,blending:THREE.AdditiveBlending,fog:false}));bluntEmberGlow.position.set(.061,0,0);bluntEmberGlow.scale.setScalar(.045);bluntRoot.add(bluntEmberGlow);
const BLUNT_REST_POSITION=new THREE.Vector3(.29,-.19,-.56),BLUNT_LIP_POSITION=new THREE.Vector3(.07,-.12,-.31);const BLUNT_REST_ROTATION=new THREE.Euler(-.22,.55,.30),BLUNT_LIP_ROTATION=new THREE.Euler(-.05,1.02,.06);
...
async function claimAvailableHarvest(){
  if(state.mode==='spectator'){if(!state.buds)return;state.buds=0;state.claimableStocks={};refresh();showToast('DEMO HARVEST CLEARED · NOTHING SENT ONCHAIN');return;}
...
async function connectPlayerWallet(){
  if(state.mode==='spectator'){showToast('SPECTATOR MODE · WALLET DISABLED');return null;}
...
async function buySelectedSeeds(){
  if(state.mode==='spectator'){state.seeds=buySpectatorSeedPack(state.seeds);refresh();showToast(`${vendorSelectedProduct.ticker} DEMO SEEDS +${vendorSelectedProduct.packSize} · SPECTATOR MODE ONLY`);return;}
...
function propertyAccessForMode(property){return state.mode==='spectator'?spectatorPropertyAccess(property):propertyAccess(property,state.walletAddress);}
function renderPropertyPanel(){
  const property=selectedProperty();if(!property)return;
  const access=propertyAccessForMode(property);ui.propertyTitle.textContent=`PROPERTY ${property.id}`;ui.propertyStatus.textContent=access==='AVAILABLE'?'AVAILABLE':access==='OWNER'?'YOUR PROPERTY':'OCCUPIED';ui.propertyCapacity.textContent=String(property.capacity);ui.propertyOwner.textContent=property.owner??'AVAILABLE — NO OWNER';
  if(state.mode==='spectator'){ui.propertyMessage.textContent=access==='OWNER'?'This demo property is stored only in this session. Enter and use its grow stations.':'Claim this property in the local demo. No wallet, token or transaction is used.';ui.propertyAction.textContent=access==='OWNER'?'OPEN DEMO PROPERTY':'CLAIM IN SPECTATOR MODE';ui.propertyAction.disabled=access==='OCCUPIED';return;}
...
let bluntInHand=null,highSession=null,lastBluntPhase='idle';const smokePuffs=[];
function spawnSmokePuff(origin,velocity,lifespan,startScale,endScale,peakOpacity){if(smokePuffs.length>=24)return;const material=new THREE.SpriteMaterial({map:bluntSpriteTexture,color:0xd8e4d2,transparent:true,opacity:peakOpacity,depthWrite:false,fog:true});const puff=new THREE.Sprite(material);puff.position.copy(origin);puff.scale.setScalar(startScale);puff.userData={velocity,life:1,lifespan,startScale,endScale,peakOpacity};scene.add(puff);smokePuffs.push(puff);}
function exhaleSmoke(){const forward=new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion),right=new THREE.Vector3(1,0,0).applyQuaternion(camera.quaternion);for(let index=0;index<10;index++){const spread=(index/10-.5)*.42;const origin=camera.position.clone().addScaledVector(forward,.34+Math.random()*.12).addScaledVector(right,spread*.5);origin.y-=.06;const velocity=forward.clone().multiplyScalar(1.55+Math.random()*.5).addScaledVector(right,spread*1.5);velocity.y+=.22+Math.random()*.2;spawnSmokePuff(origin,velocity,2.6+Math.random()*.7,.11+Math.random()*.05,.58+Math.random()*.2,.44);}}
function advanceSmoke(dt,wind){for(let index=smokePuffs.length-1;index>=0;index--){const puff=smokePuffs[index],data=puff.userData;data.velocity.multiplyScalar(Math.max(0,1-dt*1.9));data.velocity.y+=dt*.42;puff.position.x+=(data.velocity.x+wind.x)*dt;puff.position.y+=data.velocity.y*dt;puff.position.z+=(data.velocity.z+wind.z)*dt;data.life-=dt/data.lifespan;if(data.life<=0){scene.remove(puff);puff.material.dispose();smokePuffs.splice(index,1);continue;}puff.scale.setScalar(data.startScale+(data.endScale-data.startScale)*(1-data.life));puff.material.opacity=data.peakOpacity*data.life*data.life;}}
function purgeSmoke(){for(const puff of smokePuffs){scene.remove(puff);puff.material.dispose();}smokePuffs.length=0;}
function dropBlunt(){highSession=null;bluntInHand=null;lastBluntPhase='idle';bluntRoot.visible=false;bluntEmberGlow.material.opacity=0;purgeSmoke();camera.fov=70;camera.rotation.z=0;camera.updateProjectionMatrix();ui.highOverlay.classList.add('hidden');ui.highReadout.classList.remove('show');}
function smokeBlunt(){if(highSession)return showToast(`ALREADY SMOKING ${bluntInHand?.ticker??'BLUNT'}`);const ticker=pickBluntTicker({slotTicker:tickerForSlot(state.slot),claimableStocks:state.claimableStocks});const result=rollBlunt({buds:state.buds,ticker},performance.now());if(!result.changed)return showToast(`NEEDS ${BLUNT_BUD_COST} HARVESTED BUDS`);state.buds=result.buds;bluntInHand=result.blunt;highSession=beginHigh(result.blunt.potency,performance.now());bluntWrapMaterial.emissive.set(result.blunt.color);bluntRoot.visible=true;refresh();showToast(`${result.blunt.ticker} ${result.blunt.label} · SMOKING`);}
function stepBlunt(dt,now){if(!highSession)return;const sampleAt=Math.max(now,highSession.startedAt);if(sampleAt-highSession.startedAt>=HIGH_TOTAL_MS){dropBlunt();return;}const haze=sampleHigh(highSession,sampleAt);camera.fov=70+haze.fovOffset;camera.rotation.z=Math.sin(sampleAt*.0014)*haze.swayRoll;camera.updateProjectionMatrix();ui.highOverlay.classList.remove('hidden');ui.highOverlay.style.setProperty('--high-tint',haze.tint.toFixed(3));ui.highOverlay.style.setProperty('--high-vignette',haze.vignette.toFixed(3));ui.highReadout.classList.add('show');ui.highReadoutLabel.textContent=`${bluntInHand.ticker} HAZE · ${haze.secondsLeft}s`;bluntRoot.position.lerpVectors(BLUNT_REST_POSITION,BLUNT_LIP_POSITION,haze.lipReach);bluntRoot.rotation.set(BLUNT_REST_ROTATION.x+(BLUNT_LIP_ROTATION.x-BLUNT_REST_ROTATION.x)*haze.lipReach,BLUNT_REST_ROTATION.y+(BLUNT_LIP_ROTATION.y-BLUNT_REST_ROTATION.y)*haze.lipReach,BLUNT_REST_ROTATION.z+(BLUNT_LIP_ROTATION.z-BLUNT_REST_ROTATION.z)*haze.lipReach);const pulse=haze.ember*(.82+Math.sin(now*.006)*.18);bluntCherryMaterial.emissiveIntensity=.35+pulse*2.6;bluntEmberGlow.material.opacity=pulse*.78;bluntEmberGlow.scale.setScalar(.062+pulse*.046);if(haze.phase==='exhale'&&lastBluntPhase!=='exhale')exhaleSmoke();lastBluntPhase=haze.phase;}
function syncOnchainSeedSelection(){if(state.mode!=='spectator'&&state.slot>=2&&state.slot<=9&&!canEquipOnchainSeedSlot(state.onchainSnapshot,state.slot))state.slot=0;}
function syncOnchainSeedHotbar(){const visible=state.mode==='spectator'?new Set([0,1,...SEED_VARIETIES.map(({slot})=>slot)]):visibleOnchainSeedSlots(state.onchainSnapshot);document.querySelectorAll('.hotbar button').forEach(button=>{const slot=Number(button.dataset.slot);if(slot>=2&&slot<=9)button.classList.toggle('locked',!visible.has(slot));});}
function selectSlot(slot){if(state.mode!=='spectator'&&slot>=2&&slot<=9&&!canEquipOnchainSeedSlot(state.onchainSnapshot,slot)){showToast('NO SEEDS · BUY A PACK ONCHAIN');return false;}state.slot=slot;toolKick=.4;refresh();document.querySelector(`.hotbar button[data-slot="${slot}"]`)?.scrollIntoView({behavior:'smooth',block:'nearest',inline:'nearest'});return true;}
function activePot(){return state.near?.type==='pot'?state.near.pot:null;}
function activeDisplayPlant(){return state.near?.type==='displayPlant'?state.near.display:null;}
...
function startVisualWatering(pot){if(pot.growth===0){showToast('PLANT A SEED FIRST');return;}if(stageFor(pot.growth)>=5){showToast('READY FOR TRIMMERS');return;}const now=performance.now();visualWateringSession=beginVisualWatering(pot.index,now);visualWaterDropCarry=0;visualWateringLastAt=now;visualWateringCompleted=false;visualWateringUi.gauge.classList.add('show');sprayDrops(14);toolKick=1;}
function startDisplayWatering(display){const now=performance.now();visualWateringSession=beginDisplayWatering(display.displayId,now);visualWaterDropCarry=0;visualWateringLastAt=now;visualWateringCompleted=false;visualWateringUi.gauge.classList.add('show');visualWateringUi.label.textContent='HOLD TO WATER DISPLAY · 0%';sprayDrops(14);toolKick=1;}
async function useOnchainTool(pot){if(!state.walletAddress)return showToast('CONNECTED WALLET REQUIRED');const warehouse=pot.location==='warehouse';const property=activeProperty();if(!warehouse&&!property)return showToast('OWNED PROPERTY REQUIRED');const plotId=warehouse?pot.index:pot.index-WAREHOUSE_GROW_STATIONS.length;const houseId=warehouse?null:onchainPropertyId(property.id);if(state.slot===0&&wateringIsVisual(economyConfig)){startVisualWatering(pot);return;}let action,args;if(state.slot>=2){const ticker=tickerForSlot(state.slot);if(!canEquipOnchainSeedSlot(state.onchainSnapshot,state.slot)){const ownedTickers=SEED_VARIETIES.filter(({slot})=>canEquipOnchainSeedSlot(state.onchainSnapshot,slot)).map(({ticker:ownedTicker})=>ownedTicker);return showToast(ownedTickers.length?`NO ${ticker} SEEDS · SELECT ${ownedTickers.join(' / ')}`:'NO SEEDS · BUY A PACK ONCHAIN');}if(ticker==='HOOD')return showToast('HOOD STOCK TOKEN UNAVAILABLE');action=warehouse?'plantWarehouse':'plant';args=warehouse?[plotId,`0x${[...new TextEncoder().encode(ticker)].map(byte=>byte.toString(16).padStart(2,'0')).join('').padEnd(64,'0')}`]:[houseId,plotId,`0x${[...new TextEncoder().encode(ticker)].map(byte=>byte.toString(16).padStart(2,'0')).join('').padEnd(64,'0')}`];}else if(state.slot===0){action=warehouse?'waterWarehouse':'water';args=warehouse?[plotId]:[houseId,plotId];}else{action=warehouse?'claimWarehouseHarvest':'claimHarvest';args=warehouse?[plotId,state.walletAddress]:[houseId,plotId,state.walletAddress];}let result;try{result=await executeCultivationAction({action,args,account:state.walletAddress,config:economyConfig,ethereum:globalThis.ethereum});}catch(error){showToast(error?.message==='UNRESOLVED_ECONOMY_OPERATION'?'RECONCILIATION REQUIRED':`${action.toUpperCase()} FAILED SAFELY`);return;}try{await refreshOnchainState();showToast(`${action.toUpperCase()} CONFIRMED · ${result.hash.slice(0,10)}…`);}catch(error){console.warn('Stockdealer · confirmed cultivation awaiting authoritative state',error);showToast(`${action.toUpperCase()} CONFIRMED · STATE SYNC PENDING`);}}
async function useTool(){
  const display=activeDisplayPlant();
  if(display){if(state.slot!==0)return showToast('DISPLAY PLANT · EQUIP WATERING CAN');startDisplayWatering(display);return;}
  const pot=activePot();if(!pot)return;
  if(state.mode!=='spectator'&&economyConfig.economyActive&&(pot.location==='warehouse'||pot.location==='house-interior'))return useOnchainTool(pot);
  toolKick=1;
  ...
  if(state.slot===0){if(pot.growth===0)return showToast('PLANT A SEED FIRST');const next=state.mode==='spectator'?waterSpectatorPlant(pot):waterPlant(pot);if(next.wateredAt===pot.wateredAt)return showToast(pot.growth>=10?'READY FOR TRIMMERS':'ALREADY WATERED · GROWTH CLOCK ACTIVE');Object.assign(pot,next);rebuildPlant(pot);sprayDrops(14);showToast(state.mode==='spectator'?'DEMO BOOST · READY FOR TRIMMERS':'WATERED ONCE · NEXT STAGE IN 2 HOURS');}
  if(state.slot===1){const result=harvestPlant(pot);if(!result.changed)return showToast('NOT READY TO HARVEST');Object.assign(pot,result.pot);state.buds+=result.buds;state.claimableStocks[result.ticker]=(state.claimableStocks[result.ticker]||0)+result.buds;rebuildPlant(pot);showToast(`${result.ticker} HARVEST ×${result.buds} · ${state.mode==='spectator'?'DEMO INVENTORY ONLY':'STOCK CLAIM RECORDED'}`);}
  refresh();
}
...
const particles=[];
function openPanel(panel){if(state.mode==='spectator'&&panel===ui.wallet){showToast('SPECTATOR MODE · WALLET DISABLED');return;}document.exitPointerLock();panel.classList.remove('hidden');if(panel===ui.vendor)setVendorAnimation(vendorClipForPanel(true));}
...
function openHouseheadQuestion(){document.exitPointerLock();househeadPanel=nextHouseheadPanel(househeadPanel,'talk');renderHouseheadPanel();}
function openNeighborGuide(){document.exitPointerLock();closePanels();ui.neighborGuide.classList.remove('hidden');}
...
function beginSession(mode){if(multiplayerClient&&multiplayerClient.spectator!==(mode==='spectator')){multiplayerGeneration++;multiplayerClient.close();multiplayerClient=null;localServerReady=false;for(const group of remotePlayers.values())scene.remove(group);remotePlayers.clear();}state.mode=mode;ui.loaderVideo.pause();ui.gate.classList.add('hidden');ui.hud.classList.remove('hidden');document.body.classList.toggle('spectator-mode',mode==='spectator');if(mode==='player'){const assigned=assignSessionPlayerSkin(state);state.playerSkin=assigned.playerSkin;loadLocalPlayerSkin(state.playerSkin);}if(mode==='spectator'){const demo=createSpectatorSession();state.tokenBalance=demo.tokenBalance;state.seeds=demo.seeds;state.buds=demo.buds;state.claimableStocks={...demo.claimableStocks};state.properties=new Map(HOUSE_PROPERTIES.map(property=>[property.id,property]));state.activePropertyId=null;state.selectedPropertyId=null;state.slot=2;state.location=INITIAL_PLAYER_SPAWN.location;camera.position.set(INITIAL_PLAYER_SPAWN.x,1.68,INITIAL_PLAYER_SPAWN.z);yaw=INITIAL_PLAYER_SPAWN.yaw;pitch=0;for(const pot of pots){Object.assign(pot,{growth:0,water:0,seedTicker:null,plantedAt:null,wateredAt:null});rebuildPlant(pot);}ui.walletAddress.textContent=SPECTATOR_IDENTITY;}selectVendorProduct(vendorSelectedProduct);refresh();syncMultiplayerLocation();startStreetAmbience();canvas.requestPointerLock();}
...
document.querySelector('#equipSelectedSeed').addEventListener('click',()=>{const product=vendorSelectedProduct;if(!selectSlot(product.slot))return;closePanels();showToast(`${product.ticker} PACKAGE EQUIPPED`);canvas.requestPointerLock();});

function updateNear(){const nearbyProperty=state.location==='street'?propertyNear(camera.position,[...state.properties.values()]):null;if(state.location==='street'&&isNeighborGuideNearby(camera.position,STREET_LAYOUT.neighborOlder)){state.near={type:'neighbor-guide'};}else if(state.location==='street'&&isHouseheadNearby(camera.position,STREET_LAYOUT.househead)){state.near={type:'househead'};}else if(nearbyProperty){state.near={type:'property',id:nearbyProperty.id};}else{raycaster.setFromCamera(new THREE.Vector2(0,0),camera);const isVisible=object=>{for(let current=object;current;current=current.parent)if(!current.visible)return false;return true;};const hits=raycaster.intersectObjects(interactives,false).filter(hit=>{const interactive=hit.object.userData.interactive;return isVisible(hit.object)&&(interactive?.type!=='pot'||interactive.pot.location===state.location)&&(interactive?.type!=='displayPlant'||state.location==='warehouse');});state.near=hits[0]?.object.userData.interactive||null;}if(!state.near){ui.prompt.classList.remove('show');return;}let text='';if(state.near.type==='vendor')text=`[ E ] TALK TO ${VENDOR_NAME.toUpperCase()}`;else if(state.near.type==='neighbor-guide')text='[ E ] TALK TO OLD NEIGHBOR';else if(state.near.type==='househead')text='[ E ] TALK TO MEADGod';else if(state.near.type==='property'){const property=state.properties.get(state.near.id);const access=propertyAccessForMode(property);text=access==='AVAILABLE'?`[ E ] ${state.mode==='spectator'?'CLAIM DEMO':'BUY'} PROPERTY ${property.id}`:access==='OWNER'?`[ E ] OPEN PROPERTY ${property.id}`:`[ E ] OWNED BY ${compactWallet(property.owner)}`;}else if(state.near.type==='portal')text=`[ E ] ${portalFor(state.near.id)?.title||'USE DOOR'}`;else if(state.near.type==='displayPlant')text=state.slot===0?'HOLD CLICK · WATER DISPLAY PLANT':'DISPLAY PLANT · EQUIP WATERING CAN';else{const p=state.near.pot;const s=stageFor(p.growth);text=p.growth===0?'SELECT A SEED PACK [3–0] · CLICK TO PLANT':s===5?'SELECT TRIMMERS [2] · CLICK TO HARVEST':state.slot===0?'CLICK TO WATER':`${STAGES[s].name.toUpperCase()} · EQUIP WATERING CAN [1]`;}ui.prompt.textContent=text;ui.prompt.classList.add('show');}
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.04,(now-last)/1000);last=now;let playerMoving=false;
  ...
  pots.forEach(p=>{if(state.mode==='spectator'||!economyConfig.economyActive){const next=advancePlant(p);if(next.growth!==p.growth){p.growth=next.growth;rebuildPlant(p);refresh();}}if(p.plant&&!p.plant.userData.meshy)p.plant.rotation.y=Math.sin(now*.00065+p.index)*.035;});
  ...
  if(visualWateringSession){const wallDelta=Math.max(0,now-visualWateringLastAt);visualWateringLastAt=now;visualWateringSession=advanceVisualWatering(visualWateringSession,wallDelta);const stream=visualWaterDrops(visualWaterDropCarry,wallDelta/1000,particles.length);visualWaterDropCarry=stream.carry;if(stream.emitted)sprayDrops(stream.emitted);const percent=Math.round(visualWateringSession.progress*100);const displaySession='displayId' in visualWateringSession;visualWateringUi.fill.style.width=`${percent}%`;visualWateringUi.gauge.setAttribute('aria-valuenow',String(percent));visualWateringUi.label.textContent=`${displaySession?'WATERING DISPLAY':'HOLD TO WATER'} · ${percent}%`;if(visualWateringSession.complete&&!visualWateringCompleted){visualWateringCompleted=true;showToast(displaySession?'DISPLAY PLANT WATERED · VISUAL ONLY':'WATERED VISUALLY · GROWTH REMAINS ONCHAIN');}}
  ...
  advanceSmoke(dt,smokeWindFor(state.location));
  const lightZone=state.location==='street'?'street':'disabled';
  if(now-lightSlotsAt>=POINT_BUDGET.updateMs){lightSlots=assignBudgetSlots(lightSlots,lightZone,camera.position.x,camera.position.z,lightFixtures);lightSlotsAt=now;lightPlacement=budgetPlacement(lightSlots,lightZone,lightFixtures);refreshLampPools(lightPlacement);}
  lightPlacement.forEach((p,i)=>{const l=lightPool[i];l.position.set(p.x,p.y,p.z);l.color.setHex(p.color||STREET_LIGHT_POOL.color);l.distance=p.distance;l.decay=p.decay;l.intensity=p.intensity&&(p.pulse?p.intensity+Math.sin(now*.0011+p.phase*1.7)*STREET_LIGHT_POOL.flicker:p.intensity);});
  ...
  streetWalkerActors.forEach(entry=>{if(!entry.actor.visible)return;entry.patrol=advanceStrollPatrol(entry.patrol,dt,entry.walker.minZ,entry.walker.maxZ,entry.walker.speed*G60.walkerSpeed,entry.walker.dwell);entry.actor.position.z=entry.patrol.distance;const heading=entry.patrol.direction>0?0:Math.PI;const turn=Math.atan2(Math.sin(heading-entry.actor.rotation.y),Math.cos(heading-entry.actor.rotation.y));entry.actor.rotation.y+=turn*Math.min(1,dt*5);if(entry.action)entry.action.paused=!entry.patrol.moving;});
