...
function animate(now){requestAnimationFrame(animate);const dt=Math.min(.04,(now-last)/1000);last=now;let playerMoving=false;
  ...
  for(const group of remotePlayers.values()){group.visible=state.location==='street';const moving=group.position.distanceToSquared(group.userData.target)>.0001;group.position.lerp(group.userData.target,.28);const targetYaw=group.userData.targetYaw??group.rotation.y;group.rotation.y+=Math.atan2(Math.sin(targetYaw-group.rotation.y),Math.cos(targetYaw-group.rotation.y))*.28;if(group.userData.action)group.userData.action.paused=!moving;group.userData.mixer?.update(dt);}
