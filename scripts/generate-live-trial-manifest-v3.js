...
async function main(){
  const journal=await load('deployments/robinhood-mainnet-foundation-v3.json');
  ...
  const contracts=pairs.map(([name,journalName])=>{const artifact=name.startsWith('Vault_')?artifacts.Vault:artifacts[name],step=journal.steps[journalName];if(step?.status!=='completed'||!Number.isSafeInteger(step.blockNumber))throw new Error(`UNCONFIRMED_DEPLOYMENT:${journalName}`);return{name,address:journal.contracts[journalName],deploymentBlock:step.blockNumber,expectedCodeHash:journal.codeHashes[journalName],abiVersion:'3-default-warehouse-pons-trial',abi:artifact.abi};});
