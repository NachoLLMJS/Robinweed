...
contract StockdealerGameCoreV4 is Ownable2Step, ReentrancyGuard {
    ...
    function plantWarehouse(uint8 plotId, bytes32 ticker) external nonReentrant returns (bytes32 positionId) {
        ...
        warehousePlants[msg.sender][plotId] = Plant(ticker, uint64(block.timestamp), uint64(block.timestamp), positionId, sku.rewardVault);
    ...
    function plant(uint32 houseId, uint8 plotId, bytes32 ticker) external nonReentrant returns (bytes32 positionId) {
        ...
        IGameRewardVault(sku.rewardVault).consumeSeed(msg.sender, positionId);
        plants[houseId][plotId] = Plant(ticker, uint64(block.timestamp), uint64(block.timestamp), positionId, sku.rewardVault);
